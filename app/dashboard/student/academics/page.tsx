...
export default async function AcademicsPage() {
  ...
  const creditsEarned = record?.enrollments.reduce((acc, e) => acc + (e.grade && e.grade >= 60 ? e.courseSection.course.credits : 0), 0) || 0;
  const activeCredits = record?.enrollments.reduce((acc, e) => acc + (!e.grade ? e.courseSection.course.credits : 0), 0) || 0;
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              ...
              <button 
                disabled 
                className="px-4 py-2.5 bg-slate-100 text-slate-400 text-xs font-extrabold rounded-xl border border-slate-200 cursor-not-allowed flex items-center gap-2"
                title="Official Transcripts must be requested formally from the Registrar's Office"
              >
              ...
              record?.enrollments.map(e => (
                <div key={e.id} className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-slate-50 transition-colors group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-base font-bold text-slate-900">{e.courseSection.course.title}</h4>
                      <p className="text-sm font-medium text-slate-500 mt-0.5">
                        <span className="font-mono bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 mr-2">{e.courseSection.course.code}</span>
                        {e.courseSection.course.credits} Credits • {e.courseSection.semester}
                      </p>
                    </div>
                  </div>
